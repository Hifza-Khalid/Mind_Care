...
const Sessions = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'upcoming':
        return <Badge variant="default" className="bg-trust/20 text-trust border-trust/30">Upcoming</Badge>;
      case 'completed':
        return <Badge variant="secondary" className="bg-success/20 text-success border-success/30">Completed</Badge>;
      case 'cancelled':
        return <Badge variant="destructive" className="bg-severity/20 text-severity border-severity/30">Cancelled</Badge>;
      default:
            ...
            {upcomingSessions.length === 0 ? (
              <Card className="bg-card/60 backdrop-blur-sm border-border/50 shadow-soft">
                <CardContent className="p-8 text-center">
                  ...
                  <Button className="mt-4" variant="default">Book a Session</Button>
                </CardContent>
              </Card>
            ) : (
              <div className="grid gap-4 md:grid-cols-2">
                {upcomingSessions.map((session) => (
                  <Card key={session.id} className="bg-card/60 backdrop-blur-sm border-border/50 shadow-soft hover:shadow-elegant transition-all duration-300 hover:scale-[1.02]">
                    <CardHeader>
          ...
          <section>
            ...
            <div className="grid gap-4">
              {completedSessions.map((session) => (
                <Card key={session.id} className="bg-card/40 backdrop-blur-sm border-border/30 shadow-soft">
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">{session.counsellorSpecialty}</p>
                        </div>
                      </div>
