...
const AdminDashboard = () => {
  ...
  const riskLevelData = Object.entries(studentAnalytics.riskDistribution).map(([level, count]) => ({
    ...
    count,
    fill: level === 'crisis' ? '#dc2626' : level === 'high' ? '#ea580c' : level === 'medium' ? '#ca8a04' : '#16a34a'
  }));
  ...
  const moodTrendData = Object.entries(studentAnalytics.moodTrendAnalysis).map(([trend, count]) => ({
        ...
        {crisisAlerts.filter(alert => alert.status === 'active').length > 0 && (
          <Alert className="border-red-200 bg-red-50">
            ...
            <AlertDescription className="text-red-800">
              <strong>{crisisAlerts.filter(alert => alert.status === 'active').length} active crisis alerts</strong> require immediate attention.
          ...
          {stats.map((stat, index) => (
            <Card key={index} className="group hover:shadow-floating transition-all duration-500 animate-fade-in bg-white/10 backdrop-blur-xl border-white/20" style={{animationDelay: `${index * 100}ms`}}>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
                ...
                <div className={`p-3 rounded-2xl ${stat.color}/20 backdrop-blur-sm border ${stat.color.replace('bg-', 'border-')}/20 group-hover:scale-110 transition-all duration-300`}>
                  <stat.icon className={`h-6 w-6 ${stat.color.replace('bg-', 'text-')} transition-colors duration-300`} />
                </div>
              </CardHeader>
              ...
              <div className="rounded-xl bg-white/5 p-4 backdrop-blur-sm border border-white/10">
                <ResponsiveContainer width="100%" height={320}>
                  <AreaChart data={activityTimelineData}>
                    ...
                    />
                    <Area dataKey="students" stackId="1" stroke="hsl(var(--primary))" fill="hsl(var(--primary) / 0.3)" />
                    <Area dataKey="sessions" stackId="1" stroke="hsl(var(--secondary))" fill="hsl(var(--secondary) / 0.3)" />
                    <Area dataKey="chats" stackId="1" stroke="hsl(var(--accent))" fill="hsl(var(--accent) / 0.3)" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
          ...
          <CardContent className="pt-2">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-96 overflow-y-auto">
              {filteredStudents.slice(0, 9).map((student) => (
                <Card key={student.id} className="p-4 bg-white/5 border-white/20 hover:bg-white/10 transition-colors">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center space-x-2 mb-2">
                        <Badge 
                          variant="secondary" 
                          className={`text-xs ${
                            student.riskLevel === 'crisis' ? 'bg-red-500/20 text-red-600' :
                            student.riskLevel === 'high' ? 'bg-orange-500/20 text-orange-600' :
                            student.riskLevel === 'medium' ? 'bg-yellow-500/20 text-yellow-600' :
                        ...
                        <Badge 
                          variant="outline" 
                          className={`text-xs ${
                            student.moodTrend === 'improving' ? 'border-green-500 text-green-600' :
                            student.moodTrend === 'declining' ? 'border-red-500 text-red-600' :
              ...
              <div className="space-y-4">
                ...
                <div className="w-full bg-muted/50 rounded-full h-2">
                  <div 
                    ...
                    style={{width: `${Math.max(100 - platformMetrics.averageResponseTime / 2, 60)}%`}}
                  />
                </div>
          ...
          <CardContent>
            <div className="space-y-4 max-h-80 overflow-y-auto">
              {activityLogs.slice(0, 8).map((log) => (
                <div key={log.id} className="flex items-center space-x-4 p-3 rounded-lg bg-white/5 hover:bg-white/10 transition-colors">
                  <div className={`w-2 h-2 rounded-full ${
                  ...
                  <Badge 
                    variant="secondary" 
                    className={`text-xs ${
                      log.severity === 'error' ? 'bg-red-500/20 text-red-600' :
                      log.severity === 'warning' ? 'bg-yellow-500/20 text-yellow-600' :
                      log.severity === 'success' ? 'bg-green-500/20 text-green-600' :
