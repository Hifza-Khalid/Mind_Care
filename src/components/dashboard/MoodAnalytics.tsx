...
export const MoodAnalytics: React.FC<MoodAnalyticsProps> = ({
  ...
  const analytics = useMemo(() => {
    ...
    const averageMood = totalEntries > 0 
      ? filteredMoods.reduce((sum, mood) => sum + mood.mood, 0) / totalEntries 
      : 0;

    // Streak calculation
    const streak = moods.slice().reverse().reduce((currentStreak, mood, index, array) => {
      if (index === 0) return 1;
    ...
    // Best and worst days
    const bestDay = filteredMoods.reduce((best, mood) => 
      mood.mood > (best?.mood || 0) ? mood : best, null as typeof filteredMoods[0] | null
    );

    const worstDay = filteredMoods.reduce((worst, mood) => 
      mood.mood < (worst?.mood || 6) ? mood : worst, null as typeof filteredMoods[0] | null
    );
  ...
  const getInsight = () => {
    ...
    if (analytics.averageMood >= 4) {
      ...
      insights.push("You're maintaining a good balance. Consider activities that bring you joy. 🌱");
    } else {
      insights.push("Remember that tough times don't last. Consider reaching out for support. 💙");
    }

    if (analytics.streak >= 7) {
      insights.push(`Amazing ${analytics.streak}-day tracking streak! Consistency is key to self-awareness. 🔥`);
    }

    const topMood = analytics.moodDistribution[0];
    if (topMood && topMood.percentage >= 50) {
      insights.push(`You've been feeling ${topMood.label.toLowerCase()} most of the time (${topMood.percentage}% of entries).`);
    }
      ...
      <CardContent>
        <Tabs defaultValue="overview" className="w-full">
          ...
          <TabsContent value="overview" className="space-y-4">
            ...
            {/* Trend Indicator */}
            <div className={`flex items-center justify-center space-x-2 p-3 rounded-lg border ${getTrendColor()}`}>
              {getTrendIcon()}
              <span className="font-medium">
                {analytics.trend === 'improving' ? 'Mood Improving' : 
                 analytics.trend === 'declining' ? 'Needs Attention' : 'Mood Stable'}
              </span>
            </div>
          ...
          <TabsContent value="patterns" className="space-y-4">
            ...
            <div className="text-center p-3 bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-lg">
              ...
              <div className="text-2xl font-bold text-amber-600">
                <AnimatedCounter value={analytics.streak} suffix={analytics.streak !== 1 ? " days" : " day"} />
              </div>
            </div>
          </TabsContent>
                  ...
                  <div className="space-y-2">
                    {analytics.averageMood < 3 && (
                      <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
                        <div className="text-sm font-medium text-amber-800">Consider Professional Support</div>
                        <div className="text-xs text-amber-700 mt-1">
                          Your recent mood patterns suggest it might help to talk to a counselor or therapist.
                        </div>
                      </div>
                    )}
