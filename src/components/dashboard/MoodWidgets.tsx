...
import { Heart, TrendingUp, TrendingDown, Minus, Calendar, BarChart3, Clock, Zap } from 'lucide-react';
  ...
  return (
    <Card className={`hover:shadow-medium transition-all duration-300 ${className}`}>
      ...
      <CardContent className="space-y-4">
        ...
        {/* Trend Indicator */}
        <div className={`flex items-center justify-center space-x-2 p-2 rounded-lg ${getTrendColor()}`}>
          {getTrendIcon()}
          ...
          {weekDays.map((day, index) => {
            ...
            return (
              <div key={day} className="text-center">
                ...
                <div className={`h-8 w-8 mx-auto rounded-full flex items-center justify-center text-sm ${
                  isFuture 
...
const MoodQuickInsightWidget: React.FC<MoodWidgetProps> = ({ className = '', size = 'md' }) => {
  ...
  const mostFrequentMood = Object.entries(moodCounts).reduce((max, [mood, count]) => 
    count > max[1] ? [Number(mood), count] : max, [0, 0] as [number, number]
  );
    ...
    <Card className={`hover:shadow-medium transition-all duration-300 ${className}`}>
      ...
      <CardContent className="space-y-4">
        <div className="text-center p-3 bg-gradient-to-br from-primary/5 to-secondary/5 rounded-lg">
          <div className="text-sm font-medium text-muted-foreground mb-1">This week you've been</div>
